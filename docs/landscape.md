...
3. **A common evidence vocabulary.** Queue time, provider ingress, TTFT, tool timeout, route change, and packet loss must have stable semantics. The individual Agent Run Metrics draft proposes one vocabulary for run and step accounting, but has no formal IETF standing, implementation, or cross-provider interoperability evidence.
...
7. **Retry and delegation lineage.** Agent Run Metrics proposes parent and root run identifiers plus explicit delegation steps, but a retry or child agent still needs bounded, privacy-safe attribution to the original task across independent reporters and trust domains.
