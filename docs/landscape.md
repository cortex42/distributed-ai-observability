...
| [vLLM per-request metrics](https://docs.vllm.ai/en/latest/features/per_request_metrics/) | Introduced in current documentation, July 2026 | Request-level queue time, TTFT, generation time, inter-token latency, and token rate | enabling it has overhead and does not solve provider-to-network correlation |
